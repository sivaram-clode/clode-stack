// Command ec2mock is a unified local docker-deployer. On one HTTP server it
// serves three API groups against the host docker daemon:
//
...
import (
	...
	"github.com/sivaram-clode/ec2-docker-mock/internal/client/jumbo"
	"github.com/sivaram-clode/ec2-docker-mock/internal/config"
	...
	"github.com/sivaram-clode/ec2-docker-mock/internal/server"
)
...
func main() {
	...
	if err != nil {
		log.Fatalf("unified-deployer: init: %v", err)
	}
	...
	go func() {
		log.Printf("unified-deployer: listening on %s (network=%s, jumbo=%s)", cfg.Addr, cfg.Network, cfg.JumboBaseURL)
		if err := app.Listen(cfg.Addr); err != nil {
			log.Fatalf("unified-deployer: serve: %v", err)
		}
	}()
	...
	<-sig
	log.Println("unified-deployer: shutting down")
	_ = app.Shutdown()
}
